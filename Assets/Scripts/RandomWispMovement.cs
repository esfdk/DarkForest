...
public class RandomWispMovement : MonoBehaviour {
	...
	{		
		...
		tempStart.y = Terrain.activeTerrains[8].SampleHeight(tempStart) + 1f;
		start = tempStart;
		...
		moveParams.Add(iT.MoveTo.time, dist / 1.75);
	...
	{		
		...
		if(endReached)
		{
			...
			if (Vector3.Distance(pPos, transform.position) < despawnDistance)
			{
				...
				var tempEnd = end;
				tempEnd.y = Terrain.activeTerrains[8].SampleHeight(tempEnd) - 2;
	...
	{	
		...
		var ty = Terrain.activeTerrains[8].SampleHeight(newVector);
